...
var $acp_input_box = $('#project1_box'), // The box that the user types into
    ...
    // Unimplemented. ALlows the text box to get bigger.
    // This is useful. Maybe implement it later. 
        ...
        request.onreadystatechange = function() {
            if(request.readyState == 4 && request.status == 200) {
                ...
                if (acp_matches.length > 0) {
                    ...
                    displaySampledMatches (acp_sample_token, acp_matches);
 ...
 */
    ...
    //We trigger the API every time one of [space ! , . : ; ?] is pressed, but at a delay of 200ms if the last request happened too quickly.
        ...
        if (isOverflow($acp_input_box.val()))
            addBoxHeight()
        if (isUnderflow($acp_input_box.val()))
            removeBoxHeight()
        if(acp_punct_delimiter_codes.includes(event.keyCode)){
            if (this.value.length > 0)
                getMatchesAPI(this.value);
        }
        // auto_grow($acp_input_box)
        // auto_grow($acp_background_box)
    ...
    function isOverflow(text){
        ...
        if (num_lines > $acp_input_box.get()[0].rows - 1) {
            return true
        }
        return false
    }

    function isUnderflow(text){
        num_lines = text.split("\n").length
        if (num_lines < $acp_input_box.get()[0].rows - 10) {
            return true
        }
        return false
    }
    ...
    function addBoxHeight(){
        $acp_input_box.get()[0].rows += 10
        $acp_background_box.get()[0].rows += 10
    }

    function removeBoxHeight(){
        $acp_input_box.get()[0].rows -= 10
        $acp_background_box.get()[0].rows -= 10
    }
 ...
 */
 ...
 This is the basic verison of `displaySampledMatches`. However, it is unused because it is buggy.
 ...
 Note: I wrote some special forms of `displaySampledMatches` below to handle some edge cases and we use them instead.
  */
    function _displaySampledMatches (sample_token) {
...
/*  displaySampledMatches version 2
...
*/
    function displaySampledMatches (sample_token, topk_matches) {
        ...
        // We get two line breaks in a row. Thankfully, this actually rarely happens. But it needs to be handled when it does
        ...
        if (last_whitespace && last_whitespace[0].includes("\n")){ //.match(/\s+$/)){
            if (sample_token.match(/^\s+$/)){
                ...
                // console.log("pre ", candidate_matches)
                var candidate_matches = candidate_matches.filter(a => !(a[0].match(/^\s+$/))) // Disallow matches with newlines
                // console.log("post ", candidate_matches)
                ...
                // Not recommended, works poorly in practice.
        ...
        var strLength = $acp_background_box.val().length * 2;
        // $acp_input_box.focus();
        // $acp_input_box[0].setSelectionRange(strLength, strLength);
        // $acp_background_box.focus();
        $acp_background_box[0].setSelectionRange(strLength, strLength);
    }
